...
function filter_posts_button() {
  ...
  $("#result").load(remote_prefix + "/wp-admin/admin-ajax.php?action=api&v=snippet&api=cartellone-program&taxonomies=" + term_id + "&e_taxonomies=17", function(response, status, xhr) {
    if (status == "error") {
      ...
      $("#result-past").load(remote_prefix + "/wp-admin/admin-ajax.php?action=api&v=snippet&api=cartellone-past&taxonomies=" + term_id + "&e_taxonomies=17", function(response, status, xhr) {
        if (status == "error") {
          var msg = "Sorry but there was an error: ";
          $("#error").html(msg + xhr.status + " " + xhr.statusText);
        } else {
...
function show_percorsi_tax_info() {
  $('.disableable').addClass('no-more-requests');
  $("#tax-info").load(remote_prefix + "/wp-admin/admin-ajax.php?action=api&v=snippet&api=percorsi&percorso=" + term_id + "&e_taxonomies=17", function(response, status, xhr) {
    if (status == "error") {
      var msg = "Sorry but there was an error: ";
      $("#error").html(msg + xhr.status + " " + xhr.statusText);
    } else {
  ...
  if (filtro === 'percorsi' || filtro === 'abbonamenti') {
    $("#abbonamenti-list").load(remote_prefix + "/wp-admin/admin-ajax.php?action=api&v=snippet&api=elenco-abbonamenti&percorso=" + term_id + "&e_taxonomies=17", function(response, status, xhr) {
      if (status == "error") {
        var msg = "Sorry but there was an error: ";
        $("#error").html(msg + xhr.status + " " + xhr.statusText);
      } else {
